...
class StudyApp(ctk.CTkFrame):
    ...
    def letter_detail(self, parent, letter):
        ...
        for i, step_text in enumerate(steps, 1):
            ctk.CTkLabel(card, text=f"{i}. {step_text}", font=ctk.CTkFont(size=13), text_color=T.MUTED, justify="left").pack(anchor="w", padx=22, pady=2)
    ...
    def show_lesson(self, letter="D"):
        ...
        ctk.CTkButton(buttons, text="▶  Bắt đầu luyện tập", height=55, fg_color=T.BLUE, command=lambda: self.show_camera_practice(lesson.get("label", letter))).grid(row=0, column=0, sticky="ew", padx=(0, 12))
    ...
    def topic_page(self, title, subtitle, items, right_title, cta, conversation=False):
        items = items or [{"title": "Chưa có dữ liệu", "icon": "?", "desc": "Hãy thêm dữ liệu trong SQL Server.", "done": 0, "total": 1, "color": "blue", "first_label": "D"}]
        page = self._content()
        page.grid_columnconfigure(0, weight=1)
        page.grid_columnconfigure(1, weight=0)
